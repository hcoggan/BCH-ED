...
#here are the odds, with corrected p-values, that patients will be assigned to a particular category (except triage)
subset_assignment_ors <- read.csv(paste0(save_filepath, "/", method, "/caliper_", caliper_threshold, "/filtered-subset-assignment-ors-corrected-pvals.csv"))

#here are the odds, with corrected p-values, that patients will be admitted *from* that category
subset_admission_ors <- read.csv(paste0(save_filepath, "/", method, "/caliper_", caliper_threshold, "/filtered-subset-admission-ors-with-adjusted-pvals.csv"))
...
assignment_ors_here <- subset_assignment_ors %>% filter(exposure %in% c("sex_f", "race_non_hispanic_black",  "race_hispanic"), startsWith(characteristic, "triage") | startsWith(characteristic, "ed_los"))
admission_ors_here <- subset_admission_ors %>% filter(exposure %in% c("sex_f", "race_non_hispanic_black",  "race_hispanic"), startsWith(characteristic, "triage") | startsWith(characteristic, "ed_los"))
...
save_multiple_forest_plot(assignment_ors_here, create_multiple_outcome_metafor_plot, "LOS quartile", "Length of stay", paste0("ed_los_quartile_", 1:4), c("Shortest quartile", "Quartile 2", "Quartile 3", 'Longest quartile'), filename="ed_los_assign",
        log(c(0.52, 0.7, 1.35)), log(c(0.15, 5)))
...
admission_ors_here <- subset_admission_ors %>% 
    filter(exposure %in% c("sex_f", "race_non_hispanic_black",  "race_hispanic"), 
        characteristic %in% c("has_medicaid", "non_medicaid_insurance",
            ...
            startsWith(characteristic, "weight_") | startsWith(characteristic, "cts_sdi_") |
...
save_multiple_forest_plot(admission_ors_here, create_multiple_outcome_metafor_plot, "Admission to hospital", "Characteristic", 
    c("has_medicaid", "non_medicaid_insurance", paste0("weight_", c("less_than_1_sd", "1_to_2_sd", "more_than_2_sd")),
    ...
    paste0("SDI: Q", 1:4), paste0("Miles travelled: Q", 1:4), paste0("Crowding: Q", 1:4)), filename="characteristic_admission",
        log(c(0.10, 0.37, 1.5)), log(c(0.2, 2)), width=8, height=14, cex=1)